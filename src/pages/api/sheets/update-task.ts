...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { sheetId, monthSheetName, uid, number, description, date, time } = req.body;
  if (!sheetId || !monthSheetName || !uid || !number || !description || !date || !time) {
  ...
  try {
    ...
    // 2. Update the row (rowIndex+1 because Sheets API is 1-based)
    await sheets.spreadsheets.values.update({
      spreadsheetId: sheetId,
      range: `${monthSheetName}!A${rowIndex + 1}:E${rowIndex + 1}`,
      valueInputOption: "RAW",
      requestBody: {
        values: [[uid, number, description, date, time]],
      },
    });
